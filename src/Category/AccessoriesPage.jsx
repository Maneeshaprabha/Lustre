...
import { SlidersHorizontal, ChevronDown, Plus, Search, Check, ArrowLeft, ArrowRight } from 'lucide-react';
...
const AccessoriesPage = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-white font-sans text-[#1A1A1A] antialiased">
      ...
      {/* 2. UPDATED STICKY FILTER BAR (Matched to image_c73a64.png) */}
      <div className="sticky top-[88px] z-40 w-full bg-white border-y border-[#E9E3DB] px-6 md:px-12 py-5 flex justify-between items-center transition-all">
        <div className="max-w-[1500px] mx-auto w-full flex justify-between items-center">
          
          {/* Left Side: Filter Button */}
          <button className="flex items-center gap-3 text-xs md:text-sm font-bold tracking-[0.15em] uppercase text-[#1A1A1A] hover:text-[#1A1A1A]/60 transition-colors">
            <SlidersHorizontal size={16} strokeWidth={2} />
            Filters
          </button>
          
          {/* Right Side: Count & Sort By */}
          <div className="flex items-center gap-6 md:gap-10 text-[10px] md:text-xs font-bold tracking-[0.15em] uppercase">
            ...
            <button className="flex items-center gap-2 text-[#1A1A1A] hover:text-[#1A1A1A]/60 transition-colors">
              Sort By<ChevronDown size={14} strokeWidth={2} />
            </button>
          </div>
      ...
      <main className="max-w-[1500px] mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-4 gap-10 mt-12 md:mt-20 pb-24">
        
        {/* LEFT SIDEBAR: LUXURY FILTERS */}
                  ...
                  >
                    {['All Accessories', 'Bags & Totes', 'Sunglasses', 'Jewelry', 'Belts', 'Scarves', 'Small Leather Goods'].map((cat) => (
                      <button 
                        ...
                        className={`text-left py-1 text-sm font-medium transition-colors flex items-center rounded-none ${activeCategory === cat ? 'text-[#1A1A1A] font-bold' : 'text-[#1A1A1A]/60 hover:text-[#1A1A1A]'}`}
                      >
                        {cat}
                      </button>
                    ))}
                    ...
                    {['New Arrival', 'Best Seller', 'Limited Edition'].map((collection) => {
                      ...
                      return (
                        <button 
                          ...
                          className="flex items-center gap-3 text-sm group rounded-none"
                        >
                  ...
                  <AnimatePresence>
                    {isHovered === product.id && (
                      <motion.div 
                        ...
                        className="absolute inset-x-4 bottom-[60px] z-20 pointer-events-auto"
                      >
                        <button 
                          onClick={(e) => {
                            e.preventDefault();
