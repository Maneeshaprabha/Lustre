...
import { Search, ChevronDown, ChevronRight, Star, SlidersHorizontal, ArrowLeft, ArrowRight, ShoppingBag, Plus } from 'lucide-react';
    ...
    <>
      <div className="min-h-screen w-full bg-white font-sans text-[#1A1A1A] antialiased pb-24">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-4 gap-10">
          ...
          {/* RIGHT CONTENT (Square Product Grid) */}
          <div className="lg:col-span-3 flex flex-col">
            ...
            >
              {products.map((product) => (
                ...
                >
                  {/* Luxury Product Image Container (Changed to aspect-square) */}
                  <div className="relative w-full aspect-square bg-[#E9E3DB]/30 rounded-2xl overflow-hidden mb-4">
                    
                    {/* Category Tag overlay */}
                    <span className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full text-[9px] font-black tracking-widest uppercase text-[#1A1A1A] z-10 shadow-sm">
                      {product.category}
                    </span>
                    
                    {/* Changed object-top to object-center for better square framing */}
                    <img 
                      src={product.image} 
                      alt={product.name}
                    ...
                    <AnimatePresence>
                      {isHovered === product.id && (
                        ...
                        >
                          <button className="w-full bg-[#1A1A1A] text-white py-3.5 flex items-center justify-center gap-2 text-xs font-bold tracking-[0.2em] uppercase hover:bg-white hover:text-[#1A1A1A] transition-colors shadow-lg rounded-full">
                  ...
                  <div className="flex flex-col gap-1 px-1">
                    ...
                    {/* Rating row combined with reviews for a cleaner look */}
